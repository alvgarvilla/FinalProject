...
def printProtocol(packet):
	...
	if packet['protocol'] == TCP_PROTO:
		...
		if tcp_flag == SYN_FLAG :
			...
			print "ACK Receive"
			print "From IP: %s (MAC: %s)  ------->  To IP: %s (MAC: %s)" % (packet['srcip'],packet['srcmac'],packet['dstmac'],packet['dstmac'])			
			if packet['srcip'] in SYNACK_rec :
				FRIENDLY_hosts.append(packet['srcip'])
				SYNACK_rec.remove(packet['srcip'])
...
def dpi():
	q = packets()
	q.register_callback(printProtocol)
	return q
	

def addRule(self):
	self.policy = (match(srcip='10.0.0.1', dstip='10.0.0.2')  >> fwd(2))
